...
export interface PrivateSyncStatement {
  bind(...values: unknown[]): PrivateSyncStatement;
  run(): Promise<{ meta?: { changes?: number } }>;
  first<T>(): Promise<T | null>;
  all<T>(): Promise<{ results?: T[] }>;
}

export interface PrivateSyncDatabase {
  prepare(query: string): PrivateSyncStatement;
  batch(statements: PrivateSyncStatement[]): Promise<unknown>;
}

type PrivateSyncEnv = { DB?: PrivateSyncDatabase };
...
function validateWriteRequest(
  ...
  appId: string,
  isAllowedCollection: (collection: unknown) => collection is string,
): value is SyncWriteRequest {
  const expectedRevision = value.expectedRevision;
  return exactKeys(value, ["version", "appId", "collection", "recordId", "expectedRevision", "value"])
    && value.version === 1
    && value.appId === appId
    && isAllowedCollection(value.collection)
    && validRecordId(value.recordId)
    && (expectedRevision === null || (typeof expectedRevision === "number"
...
export function createPrivateAppSync(appId: string, allowedCollections: readonly string[]) {
  ...
  const ensureSchema = async (database: PrivateSyncDatabase): Promise<void> => {
    if (!schemaReady) {
  ...
  const handleGet = async (request: Request, database: PrivateSyncDatabase, owner: string): Promise<Response> => {
    ...
    if (recovery) {
      ...
      if (records.some((record) => record === null)) {
        return json({ error: "A retained legacy sync record needs review." }, 500);
      }
  ...
  const handlePut = async (request: Request, database: PrivateSyncDatabase, owner: string): Promise<Response> => {
    ...
    if (!validateWriteRequest(value, appId, isAllowedCollection)) {
