(() => {
  ...
  const legacyListToSync = (name, candidate) => {
    const olderTargetList = exactKeys(candidate, ['colors', 'positions', 'hiddenColors']);
    if (!olderTargetList && !exactKeys(candidate, [
    ...
    return {
      ...
      // The deployed Color Game predecessor saved three fields. Treat the
      // absent percentage field as an empty map so its existing named lists
      // can be migrated instead of being rejected as malformed.
      colorPercentages: olderTargetList ? Object.create(null) : entries.colorPercentages,
    };
  };
